...
//		new Student2(name, isMale, hak, ban, score)
	...
	Map<Boolean, Long> stdNumBySex =
			...
			.collect(
					partitioningBy(
							Student2::isMale,counting()));
	
	
	System.out.printf("남학생 수 %d, 여학생수 %d%n",
	...
	.collect(partitioningBy(
			Student2::isMale, collectingAndThen(
					maxBy(comparing(Student2::getScore)), Optional::get)));
	...
	= Arrays.stream(stuArr)
			.collect(partitioningBy(
					Student2::isMale, partitioningBy(s -> s.getScore() < 150)));
	List<Student2> filedMaleStu = failedStdBySex.get(true).get(true);
			List<Student2> filedFemaleStu =  failedStdBySex.get(false).get(true);	
	
	System.out.println("불합격자 목록");
	System.out.println(filedMaleStu);
	System.out.println(filedFemaleStu);
